...
- **Definition 4**  
  Two polynomials $p(x),q(x)\in S[x]$ are **equal** if $\deg(p(x))=\deg(p(x))$ (or $\deg(q(x))$ if defined) and their corresponding coefficients are equal.
...
- **Definition 5**  
  ...
  - The arithmetic properties of $S[x]$ are as follows:
    ...
    - **Remark 5.4**  
      Let $p(x),q(x)\in\mathbb K[x]$. If $p(x)q(x)=0$, then $p(x)=0\lor q(x)=0$. In fact, suppose for the sake of contradiction that $p(x)q(x)=0$ and $p(x)\neq 0\land q(x)\neq 0$. Hence we obtain that:
      $$
      \deg(p(x)q(x))=\deg(p(x))+\deg(q(x)).
      $$
      ...
      - **Proof 5.5.1**
        ...
        * **Inductive step**: Suppose that the result is true for any polynomial with degree less than or equal to $n-1$. Now we need to check if the result is true for $\deg(p(x))=n$. Let $\deg(p(x))=n$ and $\deg(s(x))=m$. We consider two cases:
          ...
          * If $n\geqslant m$, let $p(x)=a_0+a_1x+\cdots+a_nx^n$ and $s(x)=b_0+b_1x+\cdots+b_mx^m$ with $a_n, b_m \neq 0$. We construct the difference:
            $$
            \begin{aligned}
            p(x)-b_m^{-1}a_nx^{n-m}s(x)&=\left(a_0+a_1x+\cdots+a_nx^n\right)-b_m^{-1}a_nx^{n-m}\left(b_0+b_1x+\cdots +b_mx^m\right) \\
            &= \left(a_0+a_1x+\cdots+a_nx^n\right)-\left(b_m^{-1}a_nb_0x^{n-m}+\cdots+a_nx^n\right).
            \end{aligned}
            $$
